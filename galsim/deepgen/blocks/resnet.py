...
from lasagne.layers import TransposedConv2DLayer, Conv2DLayer
...
def preactivation_resnet(input_net, n_out_filters, filter_size=3, downsample=False):
    ...
    net = Conv2DLayer(net, num_filters=n_out_filters, filter_size=filter_size, stride=1, nonlinearity=elu, pad='same', W=he_norm)
    net = batch_norm(Conv2DLayer(net, num_filters=n_out_filters, filter_size=filter_size, stride=stride, nonlinearity=None, pad='same', W=he_norm))

    # Shortcut branch
    if n_filters != n_out_filters or downsample:
        shortcut = Conv2DLayer(input_net, num_filters=n_out_filters, filter_size=filter_size, nonlinearity=None, pad='same', stride=stride, W=he_norm)
    else:
        shortcut = input_net
...
def transposed_preactivation_resnet(input_net, n_out_filters, filter_size=3, upsample=False):
    ...
    if upsample:
        ...
        net = Conv2DLayer(net, num_filters=n_out_filters, filter_size=filter_size, stride=1, nonlinearity=elu, pad='same', W=he_norm)
    net = batch_norm(Conv2DLayer(net, num_filters=n_out_filters, filter_size=filter_size, stride=1, nonlinearity=None, W=he_norm, pad='same'))
    ...
    if upsample:
        ...
        shortcut = Conv2DLayer(net, num_filters=n_out_filters, filter_size=filter_size, nonlinearity=None, pad='same', stride=stride, W=he_norm)
    else:
        shortcut = input_net
