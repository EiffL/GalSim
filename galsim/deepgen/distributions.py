"""
This file is based on the parmesan package (https://github.com/casperkaae/parmesan), distributed under MIT licence.
"""
import math
...
def kl_normal2_gm2(mean1, log_var1, mean2, log_var2, weights2, eps=0.0):
    """
    Compute closed-form variational approximation of the KL-divergence between a Gaussian and Gaussian Mixture.

    .. math:
        D_{vKL} = \log \frac{1}{ \sum_i \pi_i e^{- D_{KL}(q || p_i)}}

    This formula is derived in Hershey & Olsen (2007).

    Parameters
    ----------
    mean1 : Theano tensor
        Mean of the q Gaussian, of shape (batch_size, ndim).
    log_var1 : Theano tensor
        Log variance of the q Gaussian, of shape (batch_size, ndim).
    mean2 : Theano tensor
    ...
    weigts2 : Theano tensor
        Weights of the Gaussian Mixture p, of shape (batch_size, ngaussians).
    eps : float
        Small number added to denominator to avoid NaNs.

    Returns
    -------
    Theano tensor, shape (batch_size, ndim)
        KL-divergence, already summed of input dimensions.
    """
    KLs = kl_normal2_normal2(T.shape_padright(mean1), T.shape_padright(log_var1), mean2, log_var2, eps=eps).sum(axis=1)
    return KLs.min(axis=1) - T.log(T.batched_dot(weights2, T.exp(-KLs + KLs.min(axis=1, keepdims=True))))
...
def log_gm2(x, mean, log_var, weights, eps=0.0):
    ...
    Theano tensor, shape (batch_size, ndim)
        KL-divergence, already summed of input dimensions
    """
    log_norms = log_normal2(T.shape_padright(x), mean, log_var,eps=eps).sum(axis=1)
    return log_norms.max(axis=1) + T.log(T.batched_dot(weights, T.exp(log_norms - log_norms.max(axis=1, keepdims=True))))
