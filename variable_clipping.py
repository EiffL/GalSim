...
__all__ = ["MyVariableClippingOptimizer"]


class MyVariableClippingOptimizer(optimizer.Optimizer):
  ...
  def __init__(self,
    ...
    """Construct a new clip-val optimizer.
    ...
    """
    super(MyVariableClippingOptimizer, self).__init__(use_locking, name)
    self._opt = opt
  ...
  def _clip_dense(self, var):
    with self._maybe_colocate_with(var):
      ...
      normalized_var = clip_ops.clip_by_value(
          updated_var_value, -self._max_val, self._max_val)
  ...
  def _clip_sparse(self, grad, var):
    ...
    with self._maybe_colocate_with(var):
      normalized_var_subset = clip_ops.clip_by_value(
          var_subset,- self._max_norm, self._max_norm)
